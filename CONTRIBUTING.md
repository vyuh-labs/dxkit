...
install`, `cargo update`, `go get`, edit-pom for Maven,
  ...
  - `exportDetection?` (2.7) — declares this pack's reliability for
    ...
    - the `vyuh-dxkit explore api-surface` query + the dashboard viz
      ...
      `src/analyzers/tools/graphify.ts` (Python script).
...
| `osv-scanner` ≥ 2.4.0 | Kotlin + Java + Ruby + Swift          | depVulns (Maven / RubyGems / SwiftURL — 2.4.0 is the first release whose `Package.resolved` extractor works) | `vyuh-dxkit tools install` (pinned release binary) or `go install github.com/google/osv-scanner/v2/cmd/osv-scanner@latest`          |
| `swiftlint`           | Swift                                 | lint (4.1) — the linux zip's `swiftlint-static` is self-contained                                            | `brew install swiftlint` / [GitHub release zip](https://github.com/realm/SwiftLint/releases) (see `vyuh-dxkit tools install`)       |
| `swift` 6.x           | Swift (floor execution only)          | correctness floor (`swift build` / `swift test`) — matrix rows need only swiftlint + osv-scanner             | swiftly (`https://www.swift.org/install/`) or the swift.org ubuntu24.04 tarball under `~/.local/toolchains`                         |
| `php` 8.x             | PHP                                   | phpcs runs on the ambient php CLI; the floor is `php -l`                                                     | `apt install php-cli` / `brew install php` / static build from `https://dl.static-php.dev`                                          |
| `phpcs` 4.x           | PHP                                   | lint (4.1) — PSR-12 JSON gate                                                                                | pinned phar via `vyuh-dxkit tools install` / `brew install php-code-sniffer`                                                        |
| `abaplint` 2.120.19   | ABAP                                  | lint gate + syntax floor (4.4.0) — pure npm CLI, exact pin (upstream ships no semver)                        | `npm install -g @abaplint/cli@2.120.19` (see `vyuh-dxkit tools install`)                                                            |
