...
| [`report`](commands/report.md)                                   | Full audit (report), or publish/read a score snapshot (report snapshot\|history\|trend)                    | 5-30 min                                                        |
...
| `gate`                                                           | One-shot verdict on a directory — no git, no init (fresh or tree-baseline prior)                           | < 60 sec on a package-sized tree                                |
...
| [`remediate`](commands/remediate.md)                             | Work-order remediation: recipes first, then a scoped agent, inside the verified frame                      | plan < 5 sec; a task run is budget-bounded (default 30 min cap) |
...
| `learn`                                                          | One offline HTML page: capabilities, docs, this repo’s status + setup                                      | < 10 sec                                                        |
| [`mcp`](commands/mcp.md)                                         | MCP server: dxkit’s read-only repo tools for any coding agent                                              | runs until the agent closes it                                  |
...
| `policy`                                                         | Show the effective policy (show), read values (get), change a knob (set), adopt newly shipped knobs (sync) | < 5 sec                                                         |
