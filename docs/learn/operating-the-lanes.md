...
- **`guardrail-red` / floor failed** — the frame held: the agent's change
  ...
  (a blocked diff is never a resume anchor). If the standing PR already
  holds a verified or partially-landed run awaiting your merge, the red
  draft goes to `dxkit/remediate-<task>-attempt` instead and the
  verified PR is left untouched; the log and the attempt record say so.
