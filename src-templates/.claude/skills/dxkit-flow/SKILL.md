...
**Language coverage.** Source extraction is per language pack: TypeScript/JavaScript (fetch/axios/wrappers; Express/LoopBack/NestJS/Next.js routes), Python (requests/httpx; FastAPI/Flask/Django routes — a Django `path()` route is method-agnostic and matches any verb; `re_path()` regex routes are out of scope), and Go (http.Get/Post + http.NewRequest + client wrappers; stdlib HandleFunc/Handle incl. Go 1.22 `"GET /x"` patterns, chi/echo/gin/fiber verb methods on conventional receiver names — an unusually-named router falls back to `flow.specs`). Other languages participate on the **served side** without pack coverage: point `flow.specs` at an OpenAPI document, or have the service publish its `served.json` as a workspace participant. If a user asks why a Java/Ruby service's calls don't appear in the consumed set, that's the reason — native extraction for the remaining packs is on the roadmap.
...
- **Workspace participants** — declare the services in `.dxkit/workspace.json` (`participants[]`), then:
  ...
  **Keep it fresh automatically.** `.dxkit/policy.json:flow.onMergeRefresh: true` (+ `init --with-flow-refresh`, or `update` once the policy is set) installs `dxkit-flow-refresh.yml`: after each merge it re-runs `flow publish` and LANDS the result per `flow.refreshMode`:
  - `pr` (default) — ONE standing `dxkit/flow-refresh` PR, force-updated in place, whose body is a contract-change summary. Route REMOVALS lead with a warning (once merged they arm the gate against remaining consumers — that is exactly where a human should look); additions are safe and can ride GitHub auto-merge if the team wants zero ceremony. Protected-branch-safe.
  - `push` — direct `[skip ci]` commit to the default branch. Zero ceremony, unprotected trunks only (a protected branch rejects it — use `pr`).
  The landing logic lives in the CLI (`flow publish --land=policy`), never in workflow bash.
