...
| "How has the score moved since install" | `npx vyuh-dxkit report trend` | Per-dimension score-over-time sparklines since install, from the on-merge snapshots; segmented at scoring-methodology boundaries so cross-version blips never read as real movement |
