...
Each active pack contributes a `lint:<language>` gate (e.g. `lint:typescript`, `lint:python`) using its standard zero-config linter and a stable per-line parse. Coverage is 7 of 8 built-in packs (TypeScript/JS, Python, Go, Ruby, Rust, Kotlin, C#); Java has no single zero-config standalone linter, so gate Java lint as a **user check** pointing at your build's checkstyle/PMD/spotbugs task. Tool detection is automatic through the registry — `vyuh-dxkit tools install` provisions any missing linter; a linter that isn't installed is a fail-open skip.
