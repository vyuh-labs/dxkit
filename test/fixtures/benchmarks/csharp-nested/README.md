...
`app/src/modules/Core/<Module>/<Module>.csproj` — five directory
...
| **D025f** (future) | When the direct PackageReference parser lands, this fixture's vulnerable Newtonsoft.Json 9.0.1 ref surfaces the GHSA-5crp-9r3c-p9vr advisory via the osv-scanner adhoc lockfile path. The fixture is _ready_ for that assertion now; the CI assertion lands later.                                        |

## Composition

- `app/src/modules/Core/Module/Module.csproj` — depth-5 .NET SDK
...
- `app/src/modules/Core/Module/Class1.cs` — minimal C# class with
