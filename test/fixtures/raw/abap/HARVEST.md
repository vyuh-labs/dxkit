# ABAP — raw tool-output fixture harvest

Capture real ABAP tool output and commit the bytes here. Unit
tests in `test/languages-abap.test.ts` parse these fixtures, NOT
...
TODO(abap): replace these placeholder commands with the actual capture
invocations for ABAP's tools. Run from a tiny realistic
project (commit fake credentials/known-vuln deps that surface findings).

```bash
# Example shape — adapt per tool:
# <tool> --format <fmt> <input> > test/fixtures/raw/abap/lint-output.<ext>
# <tool> --report json > test/fixtures/raw/abap/coverage-output.<ext>
# <vuln-tool> scan --format json > test/fixtures/raw/abap/depvulns-output.json
```
