...
const mockPlaybookPack = {
  ...
  // Tooling-config exemption (4.2): distinctive basename so the assertion
  // verifies the test-gap gather's exemption stays pack-driven.
  toolingConfigPatterns: ['playbook.config.*'],
...
describe('recipe playbook — synthetic pack', () => {
  ...
  // 4.2 (test-gap tooling-config exemption): the union helper + matcher stay
  // pack-driven — the synthetic pack's distinctive config basename flows
  // through `allToolingConfigPatterns`, and only for ACTIVE packs.
  it('allToolingConfigPatterns includes the mock pack patterns (4.2)', () => {
    const flags = {
      typescript: true,
      playbook: true,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } as any;
    const patterns = allToolingConfigPatterns(flags);
    expect(patterns).toContain('playbook.config.*'); // synthetic contribution
    expect(patterns).toContain('jest.config.*'); // real-pack regression guard
    expect(isToolingConfigFile('playbook.config.pbk', patterns)).toBe(true);
    expect(isToolingConfigFile('src/playbook.config.pbk', patterns)).toBe(true);
    expect(isToolingConfigFile('src/real-code.pbk', patterns)).toBe(false);
    // Inactive pack → its patterns do not leak into the union.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const tsOnly = allToolingConfigPatterns({ typescript: true } as any);
    expect(tsOnly).not.toContain('playbook.config.*');
  });
