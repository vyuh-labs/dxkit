 ...
 * then remove it. The remote-repo sibling of {@link withRefWorktree}: where that
 * checks out a ref from the CURRENT repo's object DB, this fetches a repo we do
 * NOT have locally — a cross-repo flow participant declared by a `repo:` URL.
 * Confined here per CLAUDE.md Rule 11 (the one home for "do something at a git
 * ref" checkout mechanics), so `flow publish` composes on it rather than
 * scattering clone/fetch calls.
 *
