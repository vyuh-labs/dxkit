 ...
 * matches the repo's actual PM. The INSTALL itself (frozen / resync commands,
 * tolerated fallbacks, the lockfile-sync check) is the node pack's declared
 * install strategy (`src/languages/node-install.ts`), which reads the
 * lockfile set declared here; this module keeps detection and the add /
 * upgrade command builders only.
 *
