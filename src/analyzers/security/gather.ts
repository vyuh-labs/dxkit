 ...
 */
function isAutogenBasename(relPath: string, autogenPatterns: string[]): boolean {
...
export function gatherTlsBypassFindings(cwd: string): SecurityFinding[] {
  ...
  try {
    ...
    for (const line of raw.split('\n')) {
      ...
      if (!m) continue;
      const file = m[1];
      // D056 (2.4.7): apply project `.dxkit-ignore` / `.gitignore` /
      // bundled defaults. Pre-fix only `node_modules` / `dist` were
      // hardcoded out — multi-segment dxkit-ignore paths leaked through.
      // D072 (2.4.7): apply autogen filters too (basename glob + content
      // header) so TLS-bypass findings reflect the same source-file set
      // `sourceFiles` does. Generated wrappers carrying boilerplate
      // `ServerCertificateValidationCallback` strings aren't your bypass.
      if (isExcludedPath(cwd, file)) continue;
      if (isAutogenBasename(file, autogenPatterns)) continue;
      if (isAutogeneratedByHeader(path.join(cwd, file))) continue;
