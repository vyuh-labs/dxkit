...
export function gatherGenericMetrics(cwd: string): Partial<HealthMetrics> {
  ...
  const sourceList = walkSourceFiles(cwd); // sources only (no tests, no autogen)
  ...
  // include sets via walker opts.
  const tsFiles = walkSourceFiles(cwd, { extensions: ['.ts', '.tsx', '.js', '.jsx'] });
  const pyFiles = walkSourceFiles(cwd, { extensions: ['.py'] });
  const goFiles = walkSourceFiles(cwd, { extensions: ['.go'] });
