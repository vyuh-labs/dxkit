...
// ─── Contract-domain identities (flow / schema / seam / paired / license) ────
// Live in `fingerprint-contract.ts` (split out at the large-file bar — the
// walk-globs precedent). They compose `computeContentFingerprint` above, so
// every SHA-1 scheme still resolves through this module's one primitive.
