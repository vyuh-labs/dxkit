  ...
  walk(root, (node) => {
    ...
    if (routeCalleeMatch) {
      ...
      if (raw != null && args.length >= 2 && !excluded) {
        ...
        // Ancestor group calls prefix a verb-less registration too — axum's
        // `.nest("/api", Router::new().route("/users/{id}", …))`. Packs
        // without routeGroupCallees see a null prefix (no behavior change).
        const own = normalizePath(pattern, config);
