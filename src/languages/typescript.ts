 ...
 */
function resolveTsFileAt(
  baseAbs: string,
  cwd: string,
  exists: (abs: string) => boolean = isFile,
): string | null {
  for (const ext of TS_JS_EXT) {
    if (baseAbs.endsWith(ext) && exists(baseAbs)) {
      return toRel(baseAbs, cwd);
    }
  }
  for (const ext of TS_JS_EXT) {
    if (exists(baseAbs + ext)) return toRel(baseAbs + ext, cwd);
  }
  for (const ext of TS_JS_EXT) {
    const idx = path.join(baseAbs, 'index' + ext);
    if (exists(idx)) return toRel(idx, cwd);
  }
  return null;
}
...
/**
 * Known NON-code import extensions: assets, data, styles and foreign-
 * framework single-file components a loader serves. ONLY these bail. Any
 * other dotted tail (`./users.service`, `./app.module`, `./user.dto`) is
 * part of the MODULE NAME (the NestJS / Angular convention) and keeps
 * probing as `<name>.<ext>`.
 */
const NON_CODE_IMPORT_EXT = new Set([
  '.css',
  '.scss',
  '.sass',
  '.less',
...
/** Code extensions a relative import may write explicitly; `.mts` / `.cts`
 *  are resolution-only (rarely walked as sources, but an explicit
 *  `./x.mts` import is a code import). */
...
/**
 * Generated TS/JS source the tree may legitimately not carry (a build step
 * produces it), exempted from the RELATIVE-import class only. Deliberately
 * CHECK-LOCAL, not the pack's global `autogeneratedSourcePatterns`
 * declaration: the global union feeds every walker and metric surface, and
 * changing what all of them exclude is not this check's call, so the
 * exemption's blast radius stays inside import resolution. Basename globs,
 * the same dialect the walker uses.
 */
export const TS_AUTOGEN_SOURCE_PATTERNS = [
  '*.generated.*',
  '*.gen.*',
  '*_pb.*',
  '*.pb.*',
...
/**
 * How the relative-import check sees the repo tree. The GIT view is the
 * truthful one for a floor whose question is "will this import resolve at
 * the pushed commit": a file that exists on disk but is UNTRACKED resolves
 * locally and vanishes in CI (the never-committed-file class), and a
 * git-ignored build product is the mirror image. The FILESYSTEM view is
 * the fallback for a tree that is not a git checkout, disclosed as such.
 */
export interface RepoTreeView {
  readonly source: 'git' | 'filesystem';
  /** A tracked (or, on the filesystem view, present) file at this repo-relative path. */
  isFile(rel: string): boolean;
  /** Present on disk but not tracked and not ignored: the uncommitted class. */
  isUntrackedFile(rel: string): boolean;
  /** Present on disk but git-ignored: a build product the tree does not carry. */
  isIgnoredFile(rel: string): boolean;
  /** Does ANYTHING TRACKED live at this base: the exact path, a sibling
   *  with any extension, or a directory. The false-negative floor:
   *  `./data` beside `data.json`, `./styles` beside `styles.module.css`, a
   *  `./pkg/` with a package.json main, are all shapes a bundler may serve. */
  anythingTracked(rel: string): boolean;
  /** The same question over everything the view can see (tracked,
   *  untracked, on disk): consulted LAST, after the untracked and ignored
   *  probes have had their say, so a sibling asset still resolves the
   *  question in the code's favor without hiding an uncommitted module. */
  anythingAt(rel: string): boolean;
}

/** Index a list of repo-relative paths for exact / sibling / directory lookups. */
function indexPaths(paths: readonly string[]): { files: Set<string>; stems: Set<string> } {
  const files = new Set<string>();
  const stems = new Set<string>();
  for (const p of paths) {
    files.add(p);
    const segs = p.split('/');
    for (let i = 1; i < segs.length; i++) stems.add(segs.slice(0, i).join('/'));
    const dir = segs.slice(0, -1).join('/');
    const base = segs[segs.length - 1];
    let cut = base.indexOf('.');
    while (cut > 0) {
      stems.add(dir ? `${dir}/${base.slice(0, cut)}` : base.slice(0, cut));
      cut = base.indexOf('.', cut + 1);
    }
  }
  return { files, stems };
}

function gitListZ(cwd: string, args: string[]): string[] | null {
  try {
...
/**
 * The ONE constructor of the tree view: git (tracked + untracked, ignored
 * inferred from disk) when `cwd` is a checkout, else the filesystem.
 * Exported for tests.
 */
export function repoTreeView(cwd: string): RepoTreeView {
  const tracked = gitListZ(cwd, []);
  const untracked = tracked === null ? null : gitListZ(cwd, ['--others', '--exclude-standard']);
  if (tracked === null || untracked === null) {
    return {
      source: 'filesystem',
      isFile: (rel) => isFile(path.join(cwd, rel)),
      isUntrackedFile: () => false,
      isIgnoredFile: () => false,
      anythingTracked: (rel) => anythingOnDisk(path.join(cwd, rel)),
      anythingAt: (rel) => anythingOnDisk(path.join(cwd, rel)),
    };
  }
  const t = indexPaths(tracked);
  const u = indexPaths(untracked);
  return {
    source: 'git',
    isFile: (rel) => t.files.has(rel),
    isUntrackedFile: (rel) => u.files.has(rel),
    isIgnoredFile: (rel) => !t.files.has(rel) && !u.files.has(rel) && isFile(path.join(cwd, rel)),
    anythingTracked: (rel) => t.files.has(rel) || t.stems.has(rel),
    anythingAt: (rel) =>
      t.files.has(rel) ||
      t.stems.has(rel) ||
      u.files.has(rel) ||
      u.stems.has(rel) ||
      anythingOnDisk(path.join(cwd, rel)),
  };
}

function anythingOnDisk(baseAbs: string): boolean {
...
/** One relative import the pack is prepared to judge, resolved to its
 *  repo-relative stem (extension-less when the written extension is a code
 *  extension) and the identity it would mint if missing. */
export interface TsRelativeCandidate {
  readonly spec: string;
  /** Repo-relative POSIX base path the specifier names, code extension removed. */
  readonly stemRel: string;
  /** The written code extension (`.js`, `.tsx`, ...) or '' when none. */
  readonly codeExt: string;
  readonly identity: string;
}

/** Why a relative import was NOT judged; each reason is counted and
 *  disclosed by the check. */
export type TsRelativeDecline = 'asset' | 'escapes-repo' | 'generated' | 'noise';

/**
 * The files the relative-import check judges: production TS/JS source that
 * is not a declaration file and not under the framework static-asset dir.
 * ONE predicate for the current side (the walk) and the base side (a blob
 * at a ref), so both sides share the exclusion set (Rule 2.30).
 */
export function tsJudgesFileForResolution(rel: string): boolean {
  const ext = path.posix.extname(rel);
  if (!TS_JS_EXT.includes(ext)) return false;
  if (rel.endsWith('.d.ts')) return false; // declaration files: type-only imports
  ...
  // routinely vendored UMD bundles whose internal `require` calls are the
  // bundle's own resolver, not the app's (mirror-validated).
  if (rel === 'public' || rel.startsWith('public/')) return false;
  if (matchesTestPatterns(rel, [...TS_TEST_FILE_PATTERNS, ...UNIVERSAL_TEST_DIR_PATTERNS])) {
    return false;
  }
  return !matchesAnyBasenameGlob(path.posix.basename(rel), TS_AUTOGEN_SOURCE_PATTERNS);
}

/**
 * Classify ONE relative specifier from `fromFile`: a candidate the check
 * will probe, or the reason it declines. Pure (no tree access), so the base
 * side can run it on a blob.
 */
export function tsRelativeCandidate(
  fromFile: string,
  spec: string,
): TsRelativeCandidate | TsRelativeDecline {
  if (!PLAUSIBLE_RELATIVE_SPECIFIER.test(spec)) return 'noise';
  const ext = path.posix.extname(spec);
  if (NON_CODE_IMPORT_EXT.has(ext.toLowerCase())) return 'asset';
  const rel = path.posix.normalize(path.posix.join(path.posix.dirname(fromFile), spec));
  if (rel.startsWith('../') || rel === '..') return 'escapes-repo';
  const codeExt = TS_RELATIVE_CODE_EXT.includes(ext) ? ext : '';
  const stemRel = codeExt ? rel.slice(0, -codeExt.length) : rel.replace(/\/$/, '');
  const base = path.posix.basename(stemRel);
  if (
    TS_RELATIVE_CODE_EXT.some((e) => matchesAnyBasenameGlob(base + e, TS_AUTOGEN_SOURCE_PATTERNS))
  ) {
    return 'generated';
  }
  return { spec, stemRel, codeExt, identity: projectPathIdentity(stemRel) };
}

/**
 * The project-path identities a source file's relative imports would mint
 * if their targets were missing, from content alone (no tree access), or
 * null when the pack does not judge the file. `CorrectnessProvider.
 * relativeImportIdentities`: the base-side attribution probe reads base
 * blobs through this so both sides share one extractor, one plausibility
 * filter and one exclusion set.
 */
export function tsRelativeImportIdentities(file: string, content: string): string[] | null {
  if (!tsJudgesFileForResolution(file)) return null;
  const out = new Set<string>();
  for (const spec of extractTsImportsForResolution(content)) {
    if (!(spec.startsWith('./') || spec.startsWith('../'))) continue;
    const c = tsRelativeCandidate(file, spec);
    if (typeof c !== 'string') out.add(c.identity);
  }
  return [...out];
}

/** The outcome of probing one candidate against the tree. */
export type TsRelativeOutcome =
  | { readonly kind: 'resolved' }
  | { readonly kind: 'ignored-on-disk' }
  | { readonly kind: 'missing'; readonly identity: string; readonly detail: string }
  | { readonly kind: 'untracked'; readonly identity: string; readonly detail: string };

/**
 * Probe a candidate against the tree view. Reuses the one probe
 * (`resolveTsFileAt`: exact path, extension appended, `index.*` barrel)
 * that tsconfig-path targets already go through (Rule 2), then the ESM
 * output-extension fold and the anything-exists floor, first over tracked
 * files, then over untracked ones (present locally, absent at the pushed
 * commit), then over git-ignored build products (not judged, disclosed).
 */
export function probeTsRelativeCandidate(
  c: TsRelativeCandidate,
  cwd: string,
  tree: RepoTreeView,
): TsRelativeOutcome {
  const abs = path.join(cwd, c.stemRel + c.codeExt);
  const stemAbs = path.join(cwd, c.stemRel);
  const reaches = (exists: (rel: string) => boolean): boolean => {
    const probe = (p: string) => exists(toRel(p, cwd));
    if (resolveTsFileAt(abs, cwd, probe) !== null) return true;
    for (const src of TS_OUTPUT_EXT_SOURCES[c.codeExt] ?? []) if (probe(stemAbs + src)) return true;
    for (const e of ['.mts', '.cts', '.d.ts']) if (probe(stemAbs + e)) return true;
    return false;
  };
  const anything = (q: (rel: string) => boolean): boolean =>
    q(c.stemRel) || (c.codeExt !== '' && q(c.stemRel + c.codeExt));
  if (reaches((rel) => tree.isFile(rel))) return { kind: 'resolved' };
  // A REAL module match on the untracked/ignored sets outranks the tracked
  // sibling floor below: a tracked `widget.css` beside an uncommitted
  // `widget.tsx` is the uncommitted-module class, not a served asset.
  if (reaches((rel) => tree.isUntrackedFile(rel))) {
    return {
      kind: 'untracked',
      identity: c.identity,
      detail:
        'exists on disk but is not tracked in git (uncommitted): it will be missing at the pushed commit',
    };
  }
  if (reaches((rel) => tree.isIgnoredFile(rel))) return { kind: 'ignored-on-disk' };
  if (anything((rel) => tree.anythingTracked(rel))) return { kind: 'resolved' };
  if (anything((rel) => tree.anythingAt(rel))) return { kind: 'resolved' };
  return {
    kind: 'missing',
    identity: c.identity,
    detail: tree.source === 'git' ? 'is not in the git tree' : 'does not exist on disk',
  };
}

/**
 * Whether a relative specifier (`./x`, `../x`) reaches nothing the pushed
 * tree will carry. Returns the missing target's project-path identity, or
 * null when it resolves or the pack declines to judge it. Convenience over
 * the candidate + probe pair for tests and single-specifier callers.
 */
export function unresolvedRelativeTarget(
  fromFile: string,
  spec: string,
  cwd: string,
  tree: RepoTreeView = repoTreeView(cwd),
): string | null {
  const c = tsRelativeCandidate(fromFile, spec);
  if (typeof c === 'string') return null;
  const o = probeTsRelativeCandidate(c, cwd, tree);
  return o.kind === 'missing' || o.kind === 'untracked' ? o.identity : null;
}
 ...
 *  a wholesale generated or loader-served tree. It is evaluated REPO-WIDE,
 *  before attribution, which the disclosure says: a repo carrying more
 *  dangling relative imports than this never has the class judged. */
...
/**
 * ONE quote-aware pass that strips comments AND blanks template-literal
 * bodies. Source that EMBEDS source (a scaffolder's file template, a code
 * generator's emit string) carries import statements the regex extractor
 * cannot tell from real ones, and a relative specifier resolved from the
 * embedding file is meaningless (verified on dxkit's own tree: the pack
 * scaffolder's template produced the only two relative-import findings).
 *
 * Why one pass: two blind passes re-created the bugs they each fix. A
 * comment stripper that cannot see strings deletes from a `/*` INSIDE a
 * string to a `*` `/` inside a later template (eating the template's
 * opening backtick), and a template scanner that cannot see comments lets
 * a stray backtick in a comment re-pair with a real template (dropping
 * every subsequent import and exposing the template body as real imports).
 * Here every construct is entered from the same scan: line and block
 * comments are dropped, quoted strings are copied verbatim (or dropped
 * inside a `${}` substitution, where their content is template payload,
 * not code), template bodies are blanked, and `${}` nesting is tracked to
 * its closing brace. A regex literal containing a quote or backtick is the
 * one unmodeled shape; it can only lose checks, never invent a finding.
 */
export function blankTemplateLiterals(content: string): string {
  const src = content;
  let out = '';
  let i = 0;
  const n = src.length;
  // Brace depth inside each open `${ ... }` substitution; the stack length
  // is the template nesting depth.
  const subst: number[] = [];
  ...
  while (i < n) {
    ...
    // Comments: dropped (newline kept so string handling stays line-bounded).
    if (ch === '/' && src[i + 1] === '/') {
      while (i < n && src[i] !== '\n') i++;
      continue;
    }
    ...
    if (ch === "'" || ch === '"') {
      // A quoted string: consumed as a unit, honoring escapes, ending at
      // the quote or the line (an unterminated string never swallows the
      // file). Copied in code; DROPPED inside a `${}` substitution, where
      // its content is template payload the extractor must not see.
      let j = i + 1;
      while (j < n && src[j] !== ch && src[j] !== '\n') {
        if (src[j] === '\\') j++;
        j++;
      }
      if (subst.length === 0) out += src.slice(i, j + 1);
      i = j + 1;
      continue;
    }
    if (ch === '`') {
    ...
    if (subst.length > 0) {
      if (ch === '{') subst[subst.length - 1]++;
        ...
        subst[subst.length - 1]--;
      }
      i++; // substitution code is blanked with its template
      continue;
    }
    out += ch;
    i++;
...
/**
 * The ONE extraction the import-resolution check and its base-side
 * evidence read through: one quote-aware pass (comments dropped, template
 * bodies blanked), then the pack's raw specifier capture (Rule 2: no
 * second parser).
 */
...
export function tsResolutionCheck(ctx: CorrectnessContext): ResolutionCheckResult {
  ...
  // needs. Generated files STAY in the walk: their package imports are as
  // real as anyone's; only the RELATIVE class exempts them (they import
  // other generated files a build step produces), disclosed below.
  ...
  // LAZY tree view: built on the first relative candidate that survives the
  // filters, so a repo with no judged relative imports never enumerates git
  // (the provider contract's read-only discipline; the two `git ls-files`
  // reads are the only processes this check may start).
  const treeRef: { view: RepoTreeView | null } = { view: null };
  const getTree = (): RepoTreeView => (treeRef.view ??= repoTreeView(cwd));
  const unresolved = new Map<string, string>(); // package → first importing file
  // project-path identity → first importer + the truthful detail
  const unresolvedRelative = new Map<string, { file: string; detail: string }>();
  ...
  const declined: Record<
    TsRelativeDecline | 'ignored-on-disk' | 'generated-importer' | 'untracked-importer',
    number
  > = {
    asset: 0,
    'escapes-repo': 0,
    ...
    noise: 0,
    'ignored-on-disk': 0,
    'generated-importer': 0,
    'untracked-importer': 0,
  };
  let checked = 0;
  for (const rel of files) {
    ...
    // Per-file relative-class gate, decided at the first relative candidate:
    // a generated importer's relative imports point at sibling build output
    // (not judged, counted); an UNTRACKED importer is the developer's WIP:
    // its misses must not block a pre-push or Stop while the committed work
    // is clean, so they become findings only once the file is tracked
    // (counted, disclosed). Package imports are judged either way.
    let relClass: 'ok' | 'generated-importer' | 'untracked-importer' | null = null;
    const relativeClassOf = (): 'ok' | 'generated-importer' | 'untracked-importer' => {
      if (!tsJudgesFileForResolution(rel)) return 'generated-importer';
      const tree = getTree();
      if (tree.source === 'git' && !tree.isFile(rel)) return 'untracked-importer';
      return 'ok';
    };
    for (const spec of extractTsImportsForResolution(content)) {
      ...
      if (spec.startsWith('./') || spec.startsWith('../')) {
        ...
        // class).
        const c = tsRelativeCandidate(rel, spec);
        if (typeof c === 'string') {
          declined[c]++;
          continue;
        }
        relClass ??= relativeClassOf();
        if (relClass !== 'ok') {
          declined[relClass]++;
          continue;
        }
        checked++;
        const o = probeTsRelativeCandidate(c, cwd, getTree());
        if (o.kind === 'ignored-on-disk') declined['ignored-on-disk']++;
        else if (o.kind !== 'resolved' && !unresolvedRelative.has(o.identity)) {
          unresolvedRelative.set(o.identity, { file: rel, detail: o.detail });
        }
        continue;
      }
  ...
  // Every class the check stepped back from is named (Rule 19: a partial
  // answer is disclosed, never silent), on clean, unresolved AND skipped.
  const disclosures: string[] = [];
  if (treeRef.view?.source === 'filesystem') {
    disclosures.push(
      'relative imports were resolved against the filesystem (not a git checkout): an untracked file counts as present here',
    );
  }
  const declinedParts: string[] = [];
  if (declined.asset > 0) declinedParts.push(`${declined.asset} with asset extensions`);
  if (declined['escapes-repo'] > 0) {
    declinedParts.push(`${declined['escapes-repo']} escaping the repo`);
  }
  if (declined.noise > 0) declinedParts.push(`${declined.noise} extractor-noise fragments`);
  if (declined['generated-importer'] > 0) {
    declinedParts.push(`${declined['generated-importer']} in generated importer files`);
  }
  if (declined['untracked-importer'] > 0) {
    declinedParts.push(
      `${declined['untracked-importer']} in untracked (uncommitted) importer files (they become findings once the importer is tracked)`,
    );
  }
  if (declinedParts.length > 0) {
    disclosures.push(`relative imports not judged: ${declinedParts.join(', ')}`);
  }
  if (declined.generated > 0) {
    disclosures.push(
      `${declined.generated} relative import(s) of generated source (${TS_AUTOGEN_SOURCE_PATTERNS.join(', ')}) were not judged: a build step produces them`,
    );
  }
  if (declined['ignored-on-disk'] > 0) {
    disclosures.push(
      `${declined['ignored-on-disk']} relative import(s) reach only git-ignored files on disk (build products) and were not judged: they are absent at the pushed commit unless CI regenerates them`,
    );
  }
  if (unresolvedRelative.size > MAX_CREDIBLE_UNRESOLVED_RELATIVE) {
    disclosures.push(
      `${unresolvedRelative.size} relative imports reach no file, repo-wide (the cap of ${MAX_CREDIBLE_UNRESOLVED_RELATIVE} is evaluated before attribution): that many at once suggests a generated or loader-served tree dxkit does not model, so relative imports were not judged here`,
    );
    unresolvedRelative.clear();
  }
  if (unresolved.size > MAX_CREDIBLE_UNRESOLVED) {
    const reason = `${unresolved.size} packages do not resolve: that many at once suggests a resolution mechanism dxkit does not model, not simultaneous breaks; declining rather than false-blocking`;
    // The package class steps back; the relative findings it does not
    // affect still stand, with the package decline disclosed beside them.
    if (unresolvedRelative.size === 0) return { kind: 'skipped', reason, disclosures };
    disclosures.push(`package imports were not judged: ${reason}`);
    unresolved.clear();
  }
  ...
  return {
    kind: 'unresolved',
    unresolved: [
      ...[...unresolved.entries()].map(([specifier, file]) => ({ specifier, file })),
      ...[...unresolvedRelative.entries()].map(([specifier, { file, detail }]) => ({
        specifier,
        file,
        detail,
      })),
...
export const typescript: LanguageSupport = {
  ...
  // Known tooling-config basenames that carry a JS/TS extension (4.2):
  // not test-gap candidates — nobody unit-tests a jest config. KNOWN names
  // only (false-negative bias).
