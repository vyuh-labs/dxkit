 ...
 * produces it): the pack's ONE declaration (`autogeneratedSourcePatterns`),
 * consumed by the walker's autogen filter AND the relative-import check's
 * exemption (Rules 2, 6). Basename globs, the registry shape.
 */
...
/**
 * Blank the bodies of template literals. Source that EMBEDS source (a
 * scaffolder's file template, a code generator's emit string) carries
 * import statements the regex extractor cannot tell from real ones, and a
 * relative specifier resolved from the embedding file is meaningless
 * (verified on dxkit's own tree: the pack scaffolder's template produced
 * the only two relative-import findings). Comments are stripped FIRST and
 * the scan is escape- and nesting-aware (quoted strings are skipped, `${}`
 * substitutions are tracked to their closing brace), so a stray backtick in
 * a comment or a string cannot re-pair with a later real template, which
 * would drop every subsequent import AND expose the template body as real
 * imports. A regex literal containing a backtick is the one unmodeled
 * shape; it can only lose checks.
 */
export function blankTemplateLiterals(content: string): string {
  const src = stripTsJsComments(content);
  let out = '';
  let i = 0;
  const n = src.length;
  ...
  while (i < n) {
    ...
    if (ch === "'" || ch === '"') {
      // A quoted string: copied verbatim, honoring escapes, ending at the
      // quote or the line (an unterminated string never swallows the file).
      let j = i + 1;
 ...
 * evidence read through: comments stripped, template bodies blanked, then
 * the pack's raw specifier capture (Rule 2: no second parser).
 */
...
export function tsResolutionCheck(ctx: CorrectnessContext): ResolutionCheckResult {
  ...
  // PRODUCTION source only (`tsJudgesFileForResolution`, shared with the
  // base-side probe). Test files routinely embed fixture source as string
  // literals whose import statements the regex extractor cannot tell from
  // real ones (verified on dxkit's own tree: nearly every false positive was
  // a fixture string in a test), and a test-only phantom import fails the
  ...
  const files = walkSourceFiles(cwd, {
    ...
    autogenPatterns: TS_AUTOGEN_SOURCE_PATTERNS,
  }).filter(tsJudgesFileForResolution);
  const tree = repoTreeView(cwd);
  ...
  const declined: Record<TsRelativeDecline | 'ignored-on-disk', number> = {
    asset: 0,
    'escapes-repo': 0,
    generated: 0,
    noise: 0,
  ...
  // answer is disclosed, never silent).
  const disclosures: string[] = [];
  if (tree.source === 'filesystem') {
    disclosures.push(
  ...
  if (unresolved.size > MAX_CREDIBLE_UNRESOLVED) {
    ...
    if (unresolvedRelative.size === 0) return { kind: 'skipped', reason };
...
export const typescript: LanguageSupport = {
  ...
  sourceExtensions: [...TS_JS_EXT],
  autogeneratedSourcePatterns: TS_AUTOGEN_SOURCE_PATTERNS,
